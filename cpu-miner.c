...
static const char *gbt_segwit_req =
	"{\"method\": \"getblocktemplate\", \"params\": [{\"capabilities\": "
...
start:
	...
	while (1) {
		...
		if (jsonrpc_2) {
			...
			if (have_gbt) {
				req = (char*) malloc(strlen(opt_segwit_mode? gbt_segwit_lp_req: gbt_lp_req) + strlen(lp_id) + 1);
				sprintf(req, opt_segwit_mode ? gbt_segwit_lp_req : gbt_lp_req, lp_id);
			}
