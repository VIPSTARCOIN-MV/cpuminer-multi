...
{
	const char *job_id, *prevhash, *coinb1, *coinb2, *version, *nbits, *ntime, *stateroot, *utxoroot;
	...
	stateroot = json_string_value(json_array_get(params, 8));
	utxoroot = json_string_value(json_array_get(params, 9));
	clean = json_is_true(json_array_get(params, 10));

	if (!job_id || !prevhash || !coinb1 || !coinb2 || !version || !nbits || !ntime || !stateroot || !utxoroot ||
...
#ifndef SSE
	lyra2_hash(&hash[0], &buf[0], wholeMatrix);
	printpfx("lyra2", hash);
	lyra2rev2_hash_AVX(&hash[0], &buf[0], wholeMatrix, &flag, wholeMatrix);
	printpfx("lyra2v2", hash);
	lyra2revc0ban_hash_AVX(&hash[0], &buf[0], wholeMatrix, &flag, wholeMatrix);
	printpfx("lyra2vc0ban", hash);
#else
	lyra2_hash_SSE(&hash[0], &buf[0], wholeMatrix);
	printpfx("lyra2", hash);
	lyra2rev2_hash_SSE(&hash[0], &buf[0], wholeMatrix, &flag, wholeMatrix);
	printpfx("lyra2v2", hash);
	lyra2revc0ban_hash_SSE(&hash[0], &buf[0], wholeMatrix, &flag, wholeMatrix);
	printpfx("lyra2vc0ban", hash);
#endif
