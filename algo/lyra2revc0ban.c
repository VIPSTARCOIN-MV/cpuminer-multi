...
void lyra2revc0ban_hash_AVX(void *state, const void *input, __m256i *wholeMatrix, int32_t *flag, __m256i *wholeMatrix2)
{
...
#ifndef SSE
		lyra2revc0ban_hash_AVX(hash, endiandata, wholeMatrix, &flag, wholeMatrix2);
#else
		lyra2revc0ban_hash_SSE(hash, endiandata, wholeMatrix, &flag, wholeMatrix2);
#endif
		for (int i = 0; i < 8; i++) {
